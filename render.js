...
// ---------------- ROAD TO 32 ----------------
function renderRoad32(ctx, odds) {
  ...
  document.getElementById("thirdCutoff").textContent = `~${odds.expectedThirdCutoff.pts.toFixed(1)} pts`;
...
function init() {
  ...
  const odds = computeWinOdds();
  renderHeader();
  renderLadder(ctx, odds);
  renderDraw(ctx);
  renderRoad32(ctx, odds);
  renderRivals(ctx);
  initTabs();
}
