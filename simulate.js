...
function buildStrengthModel() {
  ...
  Object.entries(stats).forEach(([team, s]) => {
    ...
    strengths[team] = {
      attack: weight * attack + (1 - weight) * leagueAvg,
      defense: weight * defense + (1 - weight) * leagueAvg,
    };
  });
...
function simScore(teamA, teamB, model) {
  ...
  const expA = Math.max(0.2, (a.attack / leagueAvg) * (b.defense / leagueAvg) * leagueAvg);
  const expB = Math.max(0.2, (b.attack / leagueAvg) * (a.defense / leagueAvg) * leagueAvg);
